...
public abstract class MicroService implements Runnable {
    ...
    private final HashMap<Class<? extends Message>, Callback> callBacks =new HashMap<Class<? extends Message>, Callback>();
    ...
    public final void run() {
        register(); // registering the microservice.
        initialize(); //init the microservice
        while (!terminated) { //simulate loop
            try {
                Message message = messageBus.awaitMessage(this); // getting a message to handle.
                Callback callback = callBacks.get(message.getClass());
                callback.call(message); // handling the message.

            } catch (Exception exception) {
                exception.printStackTrace();
            }
        }
        messageBus.unregister(this); // unregistering a microservice.
        close(); // terminate the microservice.
    }
}
